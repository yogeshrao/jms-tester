...
public class BenchmarkClient extends AbstractBenchmarkExecutionContainer {
  ...
  public BenchmarkClientInfoCommand getClientInfo() {
    if (clientInfo == null) {
      ...
      try {
        clientInfo.setClientName("bmClient-"
  ...
  public AbstractBenchmarkClient addProbeRunner(BenchmarkConfig config) {
    ...
    if (bc.prepare()) {
      ...
      log().warn(
          "Error while setting up clients for clientId: " + bc.getClientId());
      return null;
    }
  }

  public AbstractBenchmarkClient addClients(ClientType type,
  ...
  private List<AbstractBenchmarkClient> getClientsByBenchmarkId(
      String benchmarkId) {
  ...
  public void startBenchmark(StartBenchmarkCommand startCommand) {
    for (AbstractBenchmarkClient client : getClientsByBenchmarkId(startCommand
        .getBenchmarkId())) {
  ...
  public void endBenchmark(EndBenchmarkCommand endCommand) {
    for (AbstractBenchmarkClient client : getClientsByBenchmarkId(endCommand
        .getBenchmarkId())) {
      client.release();
      synchronized (activeClients) {
    ...
    getConnector().addHandler(new DefaultCommandHandler() {
      ...
      public boolean handleCommand(BenchmarkCommand command) {
        ...
        case CommandTypes.PREPARE_BENCHMARK:
          ...
          for (BenchmarkPartConfig partConfig : prepCommand
              .getBenchmarkConfig().getBenchmarkParts()) {
  ...
  public boolean matchesClient(String clientIds) {
    ...
    log()
        .debug(
            "Matching Client name (" + clientName + ") against (" + clientIds
                + ")");
    StringTokenizer sTok = new StringTokenizer(clientIds, ",");
    while (sTok.hasMoreTokens()) {
      String token = sTok.nextToken();
      if (token.equalsIgnoreCase("All")
          || getClientInfo().getClientName().matches(token)) {
        return true;
      }
    }
    return false;
