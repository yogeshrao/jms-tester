...
public class JMSCommandTransport extends AbstractCommandTransport implements
    ...
    synchronized (lock) {
      ...
      while (!started && !released) {
        if (getConnection() != null) {
          try {
            ...
            MessageConsumer consumer = session
                .createConsumer(getJmsDestinationProvider().getDestination(
        ...
        if (!started) {
          try {
            log
                .warn("Trying to reinitialize JMSTransport Connection in 5 seconds.");
            Thread.sleep(5000);
  ...
  public void sendCommand(BenchmarkCommand command) {
    ...
    if (getConnection() != null) {
      try {
        ...
        Destination dest = getJmsDestinationProvider().getDestination(
          session, getDestinationName()
        );
  ...
  public void setJmsConnectionProvider(
